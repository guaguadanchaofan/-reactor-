...
EventLoop::EventLoop(const string threaName)
{
    _isQuite = true;
    // 指定使用的调度模型（Epolldispatch）
    _dispatcher = new EpollDispatcher(this);
    _threadID = this_thread::get_id();
    _threadName = threaName == string() ? "MainThread" : threaName;
    _channelMap.clear();
    ...
    Channel *channel = new Channel(_socketpair[1], (int)FDevent::readevent, nullptr, readLocalMessage, nullptr, this);
    // channel添加到任务队列
